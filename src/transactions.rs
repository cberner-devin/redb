...
impl WriteTransaction {
    ...
    fn process_data_freed_pages_after_commit(
        ...
        let (system_root, stored_system_freed_pages, extracted_transactions) = {
            let mut system_tables = self.system_tables.lock().unwrap();
            let system_freed_pages = system_tables.system_freed_pages();
            let extracted_transactions = self.extract_freed_pages(
                &mut system_tables,
                DATA_FREED_TABLE,
                free_until,
                |page| {
    ...
    pub(crate) fn non_durable_commit(
        ...
        self.transaction_tracker.register_non_durable_commit(
            ...
            stored_data_freed_pages || stored_system_freed_pages,
        );
