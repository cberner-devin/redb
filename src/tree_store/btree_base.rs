...
impl<'a, V: Value + 'static> AccessGuard<'a, V> {
    ...
    pub(crate) fn with_arc_page(page: PageData, range: Range<usize>) -> Self {
        Self {
    ...
    pub(crate) fn arc_view(&self) -> (PageData, Range<usize>) {
        match &self.page {
            ...
            EitherPage::OwnedMemory(vec) => {
                ...
                (Arc::new(bytes.to_vec().into_boxed_slice()), 0..self.len)
            }
