...
result_dir="${1:-${repo_root}/target/branch-page-benchmark}"
mkdir -p "${result_dir}"
...
run_variant() {
    ...
    env \
        ...
        REDB_BENCHMARK_DIR="${REDB_BENCHMARK_DIR:-${result_dir}}" \
        "$@" \
        cargo bench --frozen -p redb-bench --bench redb_benchmark \
